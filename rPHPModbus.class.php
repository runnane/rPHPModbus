...
class rPHPModbus {
     ...
     */
    private $_ImplementedModbusFunctionCodes = array(
        1,
        2,
        3,
        4,
    ...
    public function __construct($host, $port = 502) {
        ...
        if (!extension_loaded('sockets')) {
            if ($this->_debug)
                echo "[--] rPHPModbus() cannot initialize, required sockets extension not loaded\n";
    ...
    public function Connect() {
        ...
        if (!$this->_host || !$this->_port) {
            if ($this->_debug)
                echo "[--] Connect() failed, missing hostname/IP\n";
        ...
        if (!$this->_socket) {
            if ($this->_debug)
                echo "[--] Connect() failed, unable to create AF_INET TCP Stream socket\n";
        ...
        if (!socket_set_option($this->_socket, SOL_SOCKET, SO_RCVTIMEO, $timeout)) {
            if ($this->_debug)
                echo "[--] Connect() failed, unable to set rcv timeoutoption on socket\n";
        ...
        if ($result === false) {
            ...
            if ($this->_debug)
                echo "[--] socket_connect() failed in rPHPModbus->Connect(): ($err)\n";
            throw new Exception("socket_connect() failed in Connect().\nReason: ($err)");
        } else {
            if ($this->_debug)
                echo "[++] Successfully connected to host {$this->_host}\n";
        }
    }
    ...
    public function DoModbusQuery($modbus_unit_identifier, $modbus_function_code, $data) {
        if ($this->_debug)
            echo "[ii] ------------- Starting new transaction -------------\n";
        $modbus_transaction_id = $this->_GetNextTransactionId();
        $modbus_protocol_identifier = 0;
        $payload = $this->_CreateModbusTCPPacket($modbus_transaction_id, $modbus_protocol_identifier,
            $modbus_unit_identifier, $modbus_function_code, $data);
        $result_raw = $this->_DoModbusPoll($payload);
        $result = $this->_ParseModbusResult($result_raw, $payload);
        if ($this->_debug)
            echo "[ii] ------------- Ending transaction -------------------\n";
        return $result;
    }
     ...
     * @throws Exception
     */
    private function _CreateModbusTCPPacket($transaction_id, $protocol_identifier, $unit_identifier,
        $modbus_function_code, $data) {
        $remaining_bytes = strlen($data) / 2 + 2;

        if (!in_array($modbus_function_code, $this->_ImplementedModbusFunctionCodes)) {
            if ($this->_debug)
                echo "[--] Modbus function code '{$modbus_function_code}' not implemented, aborting\n";
        ...
        if ($this->_debug) {
            echo "[++] Creating Modbus Packet:\n";
            echo "   header: TransactionIdentifier=[{$transaction_id}] ProtocolIdentifier=[{$protocol_identifier}] RemainingBytes=[{$hexbytecount}]\n";
            echo "   frame : UnitIdentifier=[{$unit_identifier}]\n";
            echo "           FunctionCode=[{$modbus_function_code}]\n";
            echo "           Data=[{$data}]\n";
        }
     ...
     * @return type
     */
    public function DoModbusFunction_01ReadCoilStatus($slave_address, $addr_hi, $addr_lo,
        $points_hi, $points_lo) {
        return $this->_DoModbusFunction_Basic($slave_address, 1, $addr_hi, $addr_lo, $points_hi,
            $points_lo);
    }
     ...
     * @return type
     */
    public function DoModbusFunction_02ReadInputStatus($slave_address, $addr_hi, $addr_lo,
        $points_hi, $points_lo) {
        return $this->_DoModbusFunction_Basic($slave_address, 2, $addr_hi, $addr_lo, $points_hi,
            $points_lo);
    }
     ...
     * @return type
     */
    public function DoModbusFunction_03ReadHoldingRegisters($slave_address, $addr_hi,
        $addr_lo, $points_hi, $points_lo) {
        return $this->_DoModbusFunction_Basic($slave_address, 3, $addr_hi, $addr_lo, $points_hi,
            $points_lo);
    }
     ...
     * @return type
     */
    public function DoModbusFunction_04ReadInputRegisters($slave_address, $addr_hi,
        $addr_lo, $points_hi, $points_lo) {
        return $this->_DoModbusFunction_Basic($slave_address, 4, $addr_hi, $addr_lo, $points_hi,
            $points_lo);
    }
     ...
     * @return type
     */
    public function DoModbusFunction_05WriteSingleCoil($slave_address, $addr_hi, $addr_lo,
        $value_hi, $value_lo) {
        return $this->_DoModbusFunction_Basic($slave_address, 5, $addr_hi, $addr_lo, $value_hi,
            $value_lo);
    }
     ...
     * @return type
     */
    public function DoModbusFunction_06WriteSingleRegister($slave_address, $addr_hi,
        $addr_lo, $value_hi, $value_lo) {
        return $this->_DoModbusFunction_Basic($slave_address, 6, $addr_hi, $addr_lo, $value_hi,
            $value_lo);
    }
     ...
     * @throws Exception
     */
    public function DoModbusFunction_16WriteMultipleRegisters($slave_address, $starting_address,
        $quantity_of_registers, $register_values) {
        ...
        if (strlen($subheader) != 10) {
            throw new Exception("Malformed DoModbusFunction_16WriteMultipleRegisters() subheader length, should be 10: length='" .
                strlen($subheader) . "', data='{$subheader}'");
        }
        if ($this->_debug)
            echo "[++] slave_address='{$slave_address}', quantity='{$quantity}', byte_count='{$byte_count}', register_values='{$register_values}' \n";
     ...
     * @throws Exception
     */
    public function DoModbusFunction_43ReadDeviceIdentification($slave_address, $device_id_code,
        $object_id) {
        $mei_type = "0e";
        ...
        if (strlen($payload) != 6) {
            throw new Exception("Malformed DoModbusFunction_43ReadDeviceIdentification() payload length, should be 6: length='" .
                strlen($payload) . "', data='{$payload}'");
        }

        if ($this->_debug)
            echo "[++] mei_type='{$mei_type}', dev_id='{$dev_id}', obj_id='{$obj_id}' \n";
     ...
     * @throws Exception
     */
    private function _DoModbusFunction_Basic($slave_address, $function, $addr_hi, $addr_lo,
        $points_hi, $points_lo) {
        $payload = "{$addr_hi}{$addr_lo}{$points_hi}{$points_lo}";
        if (strlen($payload) != 8) {
            throw new Exception("Malformed _DoModbusFunction_Basic() payload length, should be 8: length='" .
                strlen($payload) . "', data='{$payload}'");
        }
    ...
    private function _DoModbusPoll($request) {
        ...
        //Read data
        if ($this->_debug)
            echo "[ii] Waiting for response ... \n";
        $result = socket_read($this->_socket, 4096);
        if ($result === false) {
            if ($this->_debug)
                echo "[--] Could not read server response in _DoModbusPoll()\n";
        ...
        if ($out_transaction == $in_transaction) {
            if ($this->_debug)
                echo "[++] Got correct response _DoModbusPoll()\n";
        } else {
            if ($this->_debug)
                echo "[--] Invalid response in _DoModbusPoll() (sent: '0x{$out_transaction }', got: '0x{$in_transaction}')\n";
    ...
    private function _GetNextTransactionId() {
        $this->trid++;
        if ($this->trid == 15)
            $this->trid++; // bug in smarthouse modbus/tcp wrapper, so we avoid 0x0f transaction id
        if ($this->trid == 255)
            $this->trid = 0; // wrap
        $trid_to_send = self::Convert10to16($this->trid);
        if ($this->_debug)
            echo "[++] Got next TransactionId: '0x{$trid_to_send}'\n";
        return $trid_to_send;
    }
    ...
    private function _ParseModbusResult($result, $request) {
        ...
        if ($this->_debug)
            echo "[ii] Got packet: header=[{$header}] frame=[{$frame}]\n";
        ...
        if ($this->_debug)
            echo "[++] Got Modbus Packet:\n";
        if ($this->_debug)
            echo "   header:  TransactionIdentifier=[{$packet['header']['trid']}] ProtocolIdentifier=[{$packet['header']['protoid']}] RemainingBytes=[{$packet['header']['remaining_bytes']}]\n";
        if ($this->_debug)
            echo "   frame :  ByteCount=[{$packet['frame']['byte_count']}] UnitIdentifier=[{$packet['frame']['unit']}] FunctionCode=[{$packet['frame']['function_code']}]\n";
        if ($this->_debug)
            echo "            Data=[{$datapacket}]\n";
